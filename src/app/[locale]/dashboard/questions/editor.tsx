...
export default function QuestionEditor({ initialData, mode }: QuestionEditorProps) {
  const router = useRouter()
  const [isPreview, setIsPreview] = useState(false)
  const [tagOptions, setTagOptions] = useState<SelectOption[]>([])
  const [markdownContent, setMarkdownContent] = useState(initialData?.content || defaultMarkdown)
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      if (mode === 'create') {
        ...
        router.push(`/dashboard/questions/${initialData.id}`)
      }
    } catch (_error) {
      toast.error(mode === 'create' ? '题目保存失败' : '题目更新失败')
    }
  }
    ...
    <div className="max-w-8xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="flex items-center gap-2 text-3xl font-bold tracking-tight">
          ...
          {mode === 'create' ? '添加面试题目' : '编辑面试题目'}
        </h1>
        <p className="text-muted-foreground mt-2">
          使用 Markdown 编辑器{mode === 'create' ? '创建一道新的' : '编辑'}
          面试题目，支持代码高亮、表格、图片等丰富格式
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        {/* 内容编辑器 */}
        <Card className="min-h-[600px] w-[1200px]">
          <CardHeader>
