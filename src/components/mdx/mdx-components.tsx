...
const MDXComponents: MDXComponentsType = {
  h1: props => <h1 {...props} className={cn('mb-4 mt-6 text-3xl font-bold', props.className)} />,
  h2: props => (
    <h2
      {...props}
      className={cn('mb-4 mt-6 border-gray-200 text-2xl font-semibold', props.className)}
    />
  ),
  h3: props => (
    <h3 {...props} className={cn('text-1xl mb-4 mt-6 font-semibold', props.className)} />
  ),
  ...
  pre: ({ __rawCode__, ...props }) => {
    return (
      <pre {...props} className={cn('group relative overflow-x-auto py-4', props.className)}>
